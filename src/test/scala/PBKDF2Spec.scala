...
class PBKDF2Spec extends FlatSpec with Matchers {
  ...
  "PBKDF2" should "be consistent with the test vectors." in {
    val p1 = SymmetricKey("password".getBytes) match { case Success(k) ⇒ k; case Failure(t) ⇒ fail(t.getMessage) }
    val s1 = "salt"
    val l1 = 20
    val i1 = 1
    ...
    val p2 = SymmetricKey("password".getBytes) match { case Success(k) ⇒ k; case Failure(t) ⇒ fail(t.getMessage) }
    val s2 = "salt"
    val l2 = 20
    val i2 = 2
    ...
    val p3 = SymmetricKey("password".getBytes) match { case Success(k) ⇒ k; case Failure(t) ⇒ fail(t.getMessage) }
    val s3 = "salt"
    val l3 = 20
    val i3 = 4096
    ...
    val p4 = SymmetricKey("passwordPASSWORDpassword".getBytes) match { case Success(k) ⇒ k; case Failure(t) ⇒ fail(t.getMessage) }
