...
class MacSpec extends FlatSpec with Matchers {
  ...
  "HmacSha256" should "be consistent with the test vectors." in {
    val key1: SymmetricKey = SymmetricKey(Seq(0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b) map { _.toByte }) match { case Success(k) ⇒ k; case Failure(f) ⇒ fail(f.getMessage) }
    ...
    val key2: SymmetricKey = SymmetricKey(Seq(0x4a, 0x65, 0x66, 0x65) map { _.toByte }) match { case Success(k) ⇒ k; case Failure(f) ⇒ fail(f.getMessage) }
  ...
  it should "not fail on data with length zero." in {
    HmacSHA256(Seq(), SymmetricKey(Seq(1,2,3) map { _.toByte }) match { case Success(k) ⇒ k; case Failure(f) ⇒ fail(f.getMessage) })
  }
}
