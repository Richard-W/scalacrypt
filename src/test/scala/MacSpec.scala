...
class MacSpec extends FlatSpec with Matchers {
  ...
  "HmacSha256" should "be consistent with the test vectors." in {
    val key1: SymmetricKey = SymmetricKey(Seq(0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b) map { _.toByte })
    ...
    val key2: SymmetricKey = SymmetricKey(Seq(0x4a, 0x65, 0x66, 0x65) map { _.toByte })
  ...
  it should "not fail on keys with length zero." in {
    HmacSHA256(Seq(1, 2, 3) map { _.toByte }, SymmetricKey(Seq[Byte]()))
  }

  it should "not fail on data with length zero." in {
    HmacSHA256(Seq(), SymmetricKey(Seq(1,2,3) map { _.toByte }))
  }

  it should "have the same output for a zero key and an empty key" in {
    val k1 = SymmetricKey(Seq())
    val k2 = SymmetricKey(Seq(0, 0, 0, 0, 0) map { _.toByte })
