...
class MacSpec extends FlatSpec with Matchers {
  ...
  "HmacSha256" should "be consistent with the test vectors." in {
    val key1: Key = Key(Seq(0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b) map { _.toByte })
    ...
    val key2: Key = Key(Seq(0x4a, 0x65, 0x66, 0x65) map { _.toByte })
  ...
  it should "throw on zero length keys." in {
    try {
      HmacSHA256(Seq(1,2,3) map { _.toByte }, Key(Seq()))
    } catch {
      case f: MacException ⇒
      case _: Throwable ⇒
  ...
  it should "not fail on data with length zero." in {
    HmacSHA256(Seq(), Key(Seq(1,2,3) map { _.toByte }))
  }
}
