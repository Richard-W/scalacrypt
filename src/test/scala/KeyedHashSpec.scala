...
class KeyedHashSpec extends FlatSpec with Matchers {
  ...
  "HmacSha256" should "be consistent with the test vectors." in {
    ...
    val myMac1 = HmacSHA256(key1, data1).get
    myMac1 should be(hmac1)
    HmacSHA256.verify(key1, data1, hmac1).get should be(true)
    ...
    val myMac2 = HmacSHA256(key2, data2).get
    myMac2 should be(hmac2)
    HmacSHA256.verify(key2, data2, hmac2).get should be(true)
  }

  it should "not fail on keys with length zero." in {
    HmacSHA256(Seq[Byte]().toKey[SymmetricKeyArbitrary].get, Seq[Byte](1, 2, 3)).get
  }

  it should "not fail on data with length zero." in {
    HmacSHA256(Seq[Byte](1, 2, 3).toKey[SymmetricKeyArbitrary].get, Seq()).get
  }
  ...
  it should "have the same output for a zero key and an empty key" in {
    ...
    val h1 = HmacSHA256(k1, data).get
    val h2 = HmacSHA256(k2, data).get
    h1 should be(h2)
  }
  ...
  "The iteratee of a KeyedHash" should "be branchable." in {
    ...
    branch1Result should be(HmacSHA256(key, Seq(1, 2, 3, 4, 5, 6) map { _.toByte }).get)
    branch2Result should be(HmacSHA256(key, Seq(1, 2, 3, 7, 8, 9) map { _.toByte }).get)
  }
}
