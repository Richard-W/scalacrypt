...
class KeyedHashSpec extends FlatSpec with Matchers {
  ...
  "HmacSha256" should "be consistent with the test vectors." in {
    ...
    val myMac1 = HmacSHA256(data1, key1).get
    myMac1 should be (hmac1)
    HmacSHA256.verify(data1, hmac1, key1).get should be (true)
    ...
    val myMac2 = HmacSHA256(data2, key2).get
    myMac2 should be (hmac2)
    HmacSHA256.verify(data2, hmac2, key2).get should be (true)
  }

  it should "not fail on keys with length zero." in {
    HmacSHA256(Seq[Byte](1, 2, 3), Seq[Byte]().toKey[SymmetricKeyArbitrary].get).get
  }

  it should "not fail on data with length zero." in {
    HmacSHA256(Seq(), Seq[Byte](1,2,3).toKey[SymmetricKeyArbitrary].get).get
  }
  ...
  it should "have the same output for a zero key and an empty key" in {
    ...
    val h1 = HmacSHA256(data, k1).get
    val h2 = HmacSHA256(data, k2).get
    h1 should be (h2)
  }
  ...
  it should "be able to process iterators without consuming them." in {
    ...
    val mac = HmacSHA256(data, key).get
    var option: Option[Try[Seq[Byte]]] = None

    (HmacSHA256(seq.toIterator, key).get map({ t ⇒
      option = t._2
      t._1
    })).toSeq should be (seq)
  ...
  "The iteratee of a KeyedHash" should "be branchable." in {
    val key = Key.generate[SymmetricKeyArbitrary]
    val base = HmacSHA256(key).get.fold(Element(Seq(1,2,3) map { _.toByte }))
    ...
    branch1Result should be (HmacSHA256(Seq(1,2,3,4,5,6) map { _.toByte }, key).get)
    branch2Result should be (HmacSHA256(Seq(1,2,3,7,8,9) map { _.toByte }, key).get)
  }
}
